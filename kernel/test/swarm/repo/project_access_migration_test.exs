...
defmodule Swarm.Repo.ProjectAccessMigrationTest do
  ...
  reconstructed from the grant signatures (census FIRST), scopes are rewritten to
  `src:<uuid>`, every resolvable user's effective visibility is IDENTICAL before and after,
  break-glass survives, `down` restores the old shape, and the abort paths abort.
  """
  ...
  defp seed_old_shape! do
    ...
    for {id, name} <- [{"superuser", "Superuser"}, {"admins", "Admins"}, {"everyone", "Everyone"}] do
    ...
    INSERT INTO sso_group_map (provider, incoming_group, our_group_id) VALUES
      ('keycloak', 'everyone', 'everyone'), ('keycloak', 'admins', 'admins'), ('keycloak', 'su', 'superuser')
    """)
  ...
  test "up: grant signatures → Projects, uuid scopes, identical visibility, break-glass kept; down restores" do
    seed_old_shape!()
    assert version() == 10
    ...
    # projects from the signatures: wiki/ldap ← {admins, staff} ⇒ Internal;
    # confluence/iac/legacy ← {admins} ⇒ Operations
    assert project_of_source("wiki") == {"Internal", "shared"}
    assert project_of_source("ldap") == {"Internal", "shared"}
    assert project_of_source("confluence") == {"Operations", "shared"}
    assert project_of_source("iac") == {"Operations", "shared"}
    assert project_of_source("legacy") == {"Operations", "shared"}
    assert project_group_members("Internal") == ["admins", "staff"]
    assert project_group_members("Operations") == ["admins"]
    ...
    # identical visibility, now derived from Projects
    ...
    assert Enum.sort(Swarm.Identity.scopes_for(@carol)) == internal
    ...
    # idempotent re-run
    :ok = ProjectAccess.apply_up!(Repo)
    assert Repo.query!("SELECT count(*) FROM project").rows == [[2]]
