defmodule Swarm.SynonymyTest do
  ...
  describe "link/3 + forms/2 — reversible synonym_of, query-time resolution" do
    setup do
      canon = Store.upsert_node("concept", "Self Service Password", scope: test_src())
      alias_n = Store.upsert_node("concept", "SSP", scope: test_src())
      %{canon: canon, alias_n: alias_n}
    end

    test "links a surface form to its canonical and resolves both ways", %{} do
      assert {:ok, _} = Synonymy.link("SSP", "Self Service Password", scope: test_src())

      # querying EITHER surface form resolves to the full synonym set
      set = Synonymy.forms("SSP", [test_src()]) |> Enum.sort()
      assert set == ["SSP", "Self Service Password"]
      assert Synonymy.forms("Self Service Password", [test_src()]) |> Enum.sort() == set
    end

    test "an unlinked form resolves to just itself" do
      assert Synonymy.forms("SSP", [test_src()]) == ["SSP"]
    end

    test "reversible — unlink drops the mapping (kill-and-rebuild / rollback)" do
      {:ok, _} = Synonymy.link("SSP", "Self Service Password", scope: test_src())
      assert length(Synonymy.forms("SSP", [test_src()])) == 2
      :ok = Synonymy.unlink("SSP", "Self Service Password", scope: test_src())
      assert Synonymy.forms("SSP", [test_src()]) == ["SSP"]
    end
    ...
    test "scope-enforced — a group synonym never resolves for a public-only reader" do
      ...
      assert Synonymy.forms("SSP", ["public", test_src()]) |> length() == 2
    end
    ...
    test "link refuses a cross-scope pair (no-leak — mirror ER's cross-scope refusal)" do
      ...
      Store.upsert_node("concept", "GroupTerm", scope: test_src())
  ...
  describe "review-hardening (code review 2026-07-04)" do
    setup do
      ...
      Store.upsert_node("concept", "SS Password", scope: test_src())
      :ok
    end
    ...
    test "forms/2 is the TRANSITIVE closure — any member resolves the full set" do
      ...
      assert Synonymy.forms("SSP", [test_src()]) |> Enum.sort() == expected
      assert Synonymy.forms("SS Password", [test_src()]) |> Enum.sort() == expected
      assert Synonymy.forms("Self Service Password", [test_src()]) |> Enum.sort() == expected
    end
    ...
    test "forms/2 is type-pure — a cross-type synonym_of edge never leaks another type's key" do
      ...
      c = Store.upsert_node("concept", "SSP", scope: test_src())
      a = Store.upsert_node("article", "Some Article", scope: test_src())

      {:ok, _} =
        Store.add_edge(c, a, "synonym_of", "legacy", scope: test_src(), evidence_kind: "derived")

      refute "Some Article" in Synonymy.forms("SSP", [test_src()])
    end
  end
  ...
  describe "expand_query/3 — query-time expansion (slice 2)" do
    ...
    test "a query naming an acronym gains the canonical's surface tokens" do
      out = Synonymy.expand_query("how do i reset my SSP", [test_src()])
      assert out =~ "Self Service Password"
      assert out =~ "SSP"
    end

    test "case-insensitive match (query token 'ssp' finds node 'SSP')" do
      assert Synonymy.expand_query("reset ssp today", [test_src()]) =~ "Self Service Password"
    end

    test "no expansion when the query names no linked concept — unchanged" do
      assert Synonymy.expand_query("how do i reset my laptop", [test_src()]) ==
    ...
    test "types: expands across the concept-bearing types present (entity/article), not just concept" do
      ...
      assert Synonymy.expand_query("configure my VPN", [test_src()]) == "configure my VPN"

      assert Synonymy.expand_query("configure my VPN", [test_src()],
    ...
    test "a REFUTED synonym edge (reward<0) no longer expands (code review)" do
      ...
      assert Synonymy.expand_query("reset my SSP", [test_src()]) == "reset my SSP"
      refute Synonymy.forms("SSP", [test_src()]) == ["SSP", "Self Service Password"]
    end
    ...
    test "a multi-word form already fully present is NOT re-added (code review)" do
      ...
      out = Synonymy.expand_query("reset my Self Service Password now", [test_src()])
  ...
  describe "propose_acronyms/1 + run_acronym_pass/1 (slice 3 — automated proposer)" do
    ...
    test "a sibling_of edge auto-rejects the pair (contrastive guard, council gemini)" do
      ...
      {:ok, _} =
        Store.add_edge(a, b, "sibling_of", "contrast",
          scope: test_src(),
    ...
    test "cross-scope acronym pairs are never proposed (no-leak)" do
      Store.upsert_node("concept", "MFA", scope: test_src())
    ...
    test "run pass links only confirmed candidates (injected confirm)" do
      ...
      assert Synonymy.forms("SSP", [test_src()]) |> Enum.sort() == [
               "SSP",
    ...
    test "POLYSEMY: a short form matching two expansions in one scope proposes NEITHER" do
      ...
      Store.upsert_node("concept", "Supply Side Platform", scope: test_src())
      cands = Synonymy.propose_acronyms([])
    ...
    test "component-aware sibling guard: link refused when a sibling spans the components" do
      ...
      {:ok, _} = Synonymy.link("SSP", "Self Service Password")
      ssp2 = Store.upsert_node("concept", "Supply Side Platform", scope: test_src())
