defmodule Swarm.Graph.NeighborhoodTest do
  ...
  setup do
    ...
    grp = add_node!(%{type: "article", scope: test_src()})
    ...
    {:ok, _} = Graph.add_edge(center, grp, "mentions", "e2", reliability: 0.8, scope: test_src())
  ...
  test "in-scope neighborhood: nodes + edges within scope, private excluded", ctx do
    {:ok, r} = Neighborhood.query(ctx.center, scopes: ["public", test_src()], depth: 1)

    ids = Enum.map(r.nodes, & &1.id) |> Enum.sort()
    assert ids == Enum.sort([ctx.pub, ctx.grp])
    refute ctx.priv in ids
    assert Enum.all?(r.nodes, &(&1.scope in ["public", test_src()]))
  ...
  test "depth and node_limit are clamped; over-limit ⇒ truncated", ctx do
    ...
    {:ok, r} =
      Neighborhood.query(ctx.center, scopes: ["public", test_src()], depth: 5, node_limit: 1)
